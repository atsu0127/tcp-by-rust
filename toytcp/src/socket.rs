...
pub struct Socket {
    ...
    pub connected_connection_queue: VecDeque<SockID>, // 生成済みソケットを保持するキュー、リスニングソケットのみ使用
    pub listening_socket: Option<SockID>, // 生成元のリスニングソケット、接続済みのソケットの未使用
...
impl Socket {
    ...
    pub fn send_tcp_packet(
        ...
        dbg!("sent", &tcp_packet);
        if payload.is_empty() && tcp_packet.get_flag() == tcp_flags::ACK {
            return Ok(sent_size);
        }
